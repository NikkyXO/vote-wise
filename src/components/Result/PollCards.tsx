...
type PollResult = {
  ...
  const PollCards: React.FC<UserProps> = ({PollResultArr}) =>{
    return (
        <div className="w-[40%] bg-white rounded-lg shadow-sm p-3 flex flex-col justify-around gap-2 ">
            <p className="text-3xl font-700 my-3">Final Results</p>
            {
            ...
            <div key={index} className="p-3 flex items-center justify-between border rounded-md border-gray-600">
                <p className="text-base">{item.party}</p>
                <p className="text-base">{item.votes} votes</p>
            </div>
                ))
            }
        </div>
